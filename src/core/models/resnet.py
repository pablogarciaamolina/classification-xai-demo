...
class ResidualBlock(torch.nn.Module):
    def __init__(self, in_channels: int, out_channels: int, strides: int = 1) -> None:
        ...
        )  # Useful to simplify computations
        if self.conv_1x1:
...
class ResNet(torch.nn.Module):
    ...
    def __init__(
        self, in_channels: int, processing_structure: tuple, num_classes: int, small_inputs: bool = False
    ) -> None:
        """
        Constructor for ResNet class. Following GoogLeNet policy, ResNet separates
        ...
        Args:
            ...
            small_inputs: bool. If True, uses a smaller initial stem (3x3 conv, stride 1) for small images.
        """
        ...
        if small_inputs:
            self.ingest_block = torch.nn.Sequential(
                torch.nn.Conv2d(in_channels, 64, kernel_size=(3, 3), stride=1, padding=1),
        ...
        for module_size in latter_modules_sizes:
            ...
            m = [ResidualBlock(in_dim, out_dim, strides=2)]
            for _ in range(module_size - 1):
                m.append(ResidualBlock(out_dim, out_dim))
...
class ResNet18(ResNet):
    ...
    def __init__(self, in_channels: int, num_classes: int, small_inputs: bool = False):
        super().__init__(
            in_channels,
...
class ResNet34(ResNet):
    """
    Class wrapper of the ResNet architecture: ResNet-34
    """
